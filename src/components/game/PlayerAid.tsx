...
export function PlayerAid({ isOpen, onClose, standalone = false }: PlayerAidProps) {
    if (!isOpen && !standalone) return null;

    const modalContent = (
        <div className={`player-aid-modal ${standalone ? 'standalone' : ''}`} onClick={e => e.stopPropagation()}>
            {!standalone && <button className="player-aid-close" onClick={onClose}>×</button>}
